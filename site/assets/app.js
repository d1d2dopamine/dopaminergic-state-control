...
async function overview(){
  const [findings, reviewQueue, run, synapses] = await Promise.all([getJSON('data/findings.json'), getJSONOptional('data/review_queue.json'), getJSON('data/run.json'), getJSONOptional('data/synapses.json')]);
...
async function networkPage(){
  const [findings,synapses]=await Promise.all([getJSON('data/findings.json'),getJSONOptional('data/synapses.json')]); const select=document.querySelector('#network-finding');
